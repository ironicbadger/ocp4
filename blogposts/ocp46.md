This blog post discusses installation of OpenShift Container Platform (OCP) 4.6 on VMware with user provisioned infrastructure (UPI). The approaches outlined here make use of new features in both Terraform and Red Hat CoreOS (RHCOS) and aims to equip with the knowledge required to implement and use Infrastructure as Code principles to deploy your clusters.

The [IronicBadger/ocp4](https://github.com/IronicBadger/ocp4) contains the code referenced throughout this post. I wrote previously about install OpenShift 4.3 in April [here](https://www.openshift.com/blog/installing-ocp-4.3-on-vmware-with-upi) if you're curious to see how the process is maturing over time.

The following content is not intended for production usage without modification. In other words, this post should help you understand some of the concepts and technical requirements to deploy OCP 4.6 for the first time before adapting them to your own needs.
...
One of the first things to take into consideration is picking a "cluster id" (in this case we are using `ocp46` - referred to interchangeably as `clusterid` or `cluster_slug` throughout). The `clusterid` uniquely identifies each OpenShift 4 cluster in your domain and also becomes part of the cluster's FQDN. This will be set in both your Terraform `terraform.tfvars` variables file as well as `install-config.yaml`.

In order to remove any external dependencies on deploying OCP4, this repo now includes [CoreDNS](https://coredns.io/). Configure the IPs, cluster slug and domain you'd like to use in `ocp4/clusters/4.6-coredns-and-staticIP/terraform.tfvars`.

As this repo is configured to use static IPs, removing the provided CoreDNS implementation is as simple as specifying a different dns server. Note that there are two DNS server addresses required, one is a public DNS so that the bootstrap, CoreDNS and loadbalancer nodes can reach quay.io to pull images and the other is the IP of the CoreDNS VM. Once the CoreDNS VM is up, it will forward external DNS queries on to the public provider configured - an unfortunate chicken and egg problem. 

# Static IPs, Ignition and Afterburn

With prior releases DHCP was required. However having nodes changes IPs after cluster creation was not a good idea so we ended up recommending MAC addressed based reservations. This enable nodes to still get an IP via DHCP but we'd know what the IP was going to be ahead of time. However, with some new changes in RHCOS 4.6 static IPs can now be reliably implemented by providing arguments to dracut at boot.

With Red Hat CoreOS 4.6 Ignition v3 has been adopted. At the time of writing only the `community-terraform-providers/ignition` provider supports the new Ignition v3 spec. 
...
A companion to Ignition is [Afterburn](https://github.com/coreos/afterburn), a tool which simplifies injecting network command-line arguments such as setting a static IP.

In Terraform we are able to use the [`extra_config`](https://registry.terraform.io/providers/hashicorp/vsphere/latest/docs/resources/virtual_machine#extra_config) option to pass in both the ignition config  as well as set the required kernel arguments. An example of this is shown in the `rhcos-static` module [here](https://github.com/IronicBadger/ocp4/blob/master/modules/rhcos-static/main.tf#L61). The formatting of the arguments that `afterburn.initrd.network-kargs` requires is documented in the dracut manpages [here](https://www.man7.org/linux/man-pages/man7/dracut.cmdline.7.html).
...
I wrote in more detail about Terraform 0.13 [on my personal blog](https://blog.ktz.me/terraform-0-13-count-modules/).

# Installation

It's now time to gather the installation artifacts. Visit [try.openshift.com](https://try.openshift.com) and go to the vSphere section to get what you need, which is:
...
Now comes the fun part, we can create the VMs and therefore the cluster. Terraform is doing all the hard work here. Change into the directory containing the Terraform definitions of the cluster, in my case this is `ocp4/clusters/4.6-coredns-and-staticIP` and then run `terraform apply`. Accept the prompt by typing `yes` and about 90 seconds later, your infrastructure should have been created.
